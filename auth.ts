...
export async function sendEmailVerification(conn: SQL, email: string) {
  ...
  if (users.length === 0) {
    return {
      message:
        "If this email is registered or unverified, a code has been sent.",
    };
  }
  const user = users[0];
...
export default function registerAuthRoutes(app: OpenAPIHono, conn: SQL) {
  // Register
  app.openapi(routes.registerRoute, async (c) => {
    try {
  ...
  // Send Email Verify
  app.openapi(routes.sendEmailVerifyRoute, async (c) => {
    const body = await c.req.parseBody();
    const email = String(body.email || "");
    if (!email) {
  ...
  // Verify Email
  app.openapi(routes.verifyEmailRoute, async (c) => {
    const body = await c.req.parseBody();
    ...
    await conn`
      UPDATE users SET email_verified = true WHERE id = ${(record.userId as any).id};
    `;
    return c.json({ message: "Email has been verified" });
  });
  ...
  // Authenticate
  app.openapi(routes.authenticateRoute, async (c) => {
    const body = await c.req.parseBody();
  ...
  // Password Reset Code
  app.openapi(routes.passwordResetCodeRoute, async (c) => {
    try {
      const body = await c.req.parseBody();
      const email = String(body.email || "");
      if (!email) {
  ...
  // Verify Reset Code
  app.openapi(routes.verifyResetCodeRoute, async (c) => {
    const body = await c.req.parseBody();
    const code = String(body.code || "");
  ...
  // Reset Password
  app.openapi(routes.resetPasswordRoute, async (c) => {
    const body = await c.req.parseBody();
  ...
  // Refresh
  app.openapi(routes.refreshRoute, async (c) => {
    const body = await c.req.parseBody();
  ...
  // Logout
  app.openapi(routes.logoutRoute, async (c) => {
    const body = await c.req.parseBody();
    const refreshToken = String(body.refreshToken || "");
    if (!refreshToken) return c.json({ error: "Missing refresh token" }, 400);
